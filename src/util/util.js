...
// 生成随机4位验证码
export const randomCaptchaCode = (len = 4) => {
  let random = ''
  let codeStr = 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789';
  for (let i = 0; i < len; i++) {
    let ran = Math.floor(Math.random() * (0 - 63) + 63);
    random += codeStr.charAt(ran)
  }
  return random.split('').map(item => ({
    str: item,
