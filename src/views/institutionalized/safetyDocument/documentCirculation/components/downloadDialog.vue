      ...
      <template #content>
        ...
        >
          <el-tab-pane name="download">
            <span slot="label"
              ><i class="icon-a-zu19815 tab-icon"></i>已下载人员<span
                class="num"
                >{{ downloadNum }}</span
              ></span
            >
            <person-info ref="downloadInfoRef"></person-info>
          </el-tab-pane>
          <el-tab-pane name="undownload">
            <span slot="label"
              ><i class="icon-a-zu19815 tab-icon"></i>未下载人员<span
                class="num"
                >{{ undownloadNum }}</span
              ></span
            >
            <person-info ref="undownloadInfoRef"></person-info>
          </el-tab-pane>
        </el-tabs>
      </template>
      <template #bottom>
...
export default {
  ...
  methods: {
    handleClick(tab) {
      switch (tab.name) {
        case "download":
          this.getdownloadInfo(this.row.id);
          break;
        case "undownload":
          this.getUndownloadInfo(this.row.id);
          break;
      }
    },
    ...
    async getdownloadInfo() {
      this.$refs.downloadInfoRef.tableData = [
        {
          dept: "生产部",
          name: "王明阳",
        },
    ...
    // 获取识别信息
    async getUndownloadInfo() {
      this.$refs.undownloadInfoRef.tableData = [
        {
          dept: "生产部",
          name: "王大锤",
        },
