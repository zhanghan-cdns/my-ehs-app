...
import { ranksPageAPI, ranksDeleteAPI } from "@/api/emergency/ranks";
...
export default {
      ...
      columns: [
        ...
        },
        {
          ...
          render: (h, { row, column, $index }) => {
            return <span>{row.ranksType ? row.ranksType.value : ""}</span>;
          },
        },
        {
  ...
  methods: {
    ...
    async getTableData() {
      ...
      const res = await ranksPageAPI(params);
      if (res.code !== 200) {
    ...
    deleteTableData() {
      this.$confirm("你确定要删除此条记录吗?", "提示", {
        ...
        ranksDeleteAPI({ ids: this.ids.join(",") }).then((res) => {
          if (res && res.code === 200) {
            this.getTableData();
