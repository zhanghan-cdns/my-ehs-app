...
import {
  disposalTechnologyPageAPI,
  disposalTechnologyDeleteAPI,
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        ...
        {
          field: "accidentDisposalTechnology",
          title: "事故应急处置技术",
          align: "center",
        },
        {
  ...
  methods: {
    ...
    async getTableData() {
      ...
      const res = await disposalTechnologyPageAPI(params);
      if (res.code !== 200) {
    ...
    deleteTableData() {
      this.$confirm("你确定要删除此条记录吗?", "提示", {
        ...
        disposalTechnologyDeleteAPI({ ids: this.ids.join(",") }).then((res) => {
          if (res && res.code === 200) {
            this.getTableData();
