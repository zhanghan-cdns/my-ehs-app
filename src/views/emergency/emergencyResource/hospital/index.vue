...
import { hospitalPageAPI, hospitalDeleteAPI } from "@/api/emergency/hospital";
...
export default {
      ...
      columns: [
        ...
        },
        {
          ...
          render: (h, { row, column, $index }) => {
            return <span>{row.level ? row.level.value : ""}</span>;
          },
        },
        {
        ...
        {
          field: "mainResponsiblePersonNumber",
          title: "主要负责人电话",
          align: "center",
        },
        {
  ...
  methods: {
    ...
    async getTableData() {
      ...
      const res = await hospitalPageAPI(params);
      if (res.code !== 200) {
    ...
    deleteTableData() {
      this.$confirm("你确定要删除此条记录吗?", "提示", {
        ...
        hospitalDeleteAPI({ ids: this.ids.join(",") }).then((res) => {
          if (res && res.code === 200) {
            this.getTableData();
