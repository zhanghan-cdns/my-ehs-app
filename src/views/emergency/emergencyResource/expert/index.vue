...
import { expertPageAPI, expertDeleteAPI } from "@/api/emergency/expert";
...
export default {
      ...
      columns: [
        ...
        },
        {
          ...
          render: (h, { row, column, $index }) => {
            return (
              <span>{row.technicalTitle ? row.technicalTitle.value : ""}</span>
            );
          },
        },
        {
  ...
  methods: {
    ...
    async getTableData() {
      ...
      const res = await expertPageAPI(params);
      if (res.code !== 200) {
    ...
    deleteTableData() {
      this.$confirm("你确定要删除此条记录吗?", "提示", {
        ...
        expertDeleteAPI({ ids: this.ids.join(",") }).then((res) => {
          if (res && res.code === 200) {
            this.getTableData();
