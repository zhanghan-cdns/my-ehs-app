...
<script>
import {roomCreateAPI, roomViewAPI, roomUpdateAPI} from '@/api/enterprise/room/index';
...
export default {
  ...
  methods: {
    ...
    async add() {
      try {
        const res = await roomCreateAPI({...this.form, stoSto: this.tableData});
        if (res.code === 200) {
          this.close()
          this.$emit('refresh')
        }
    ...
    async modify() {
      try {
        const res = await roomUpdateAPI({...this.form, id: this.rowData.id, stoSto: this.tableData});
        if (res.code === 200) {
          this.close()
          this.$emit('refresh')
        }
