...
import {
  productionEquipmentListAPI,
  productionEquipmentDeleteAPI,
...
export default {
  ...
  methods: {
    ...
    async getTableData({ query, domain }) {
      ...
      const res = await productionEquipmentListAPI(params);
      if (res.code === 200) {
    ...
    async deleteData(row) {
      this.$confirm("确定删除该条数据吗?", "提示", {
        ...
        .then(async () => {
          const res = await productionEquipmentDeleteAPI({
            ids: row.id,
          });
          if (res.code === 200) {
