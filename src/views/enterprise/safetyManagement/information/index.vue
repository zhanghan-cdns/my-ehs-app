      ...
      columns: [
        ...
        },
        {
          ...
          render: (h, { row, column, $index }) => {
            return (
              <span>{row.menuDifferent ? row.menuDifferent.value : ""}</span>
            );
          },
        },
        {
          field: "typeForeignKey",
          title: "装置设施型号",
          align: "center",
        },
        {
        ...
        },
        {
          ...
          render: (h, { row, column, $index }) => {
            if (row.maintenanceStatus) {
              return (
                <span>{row.maintenanceStatus === 1 ? "完好" : "维修中"}</span>
              );
            } else {
              return <span></span>;
            }
  ...
  methods: {
    ...
    async deleteData(row) {
      this.$confirm("确定删除该条数据吗?", "提示", {
        ...
        .then(async () => {
          const res = await informationDeleteAPI({
            ids: row.id,
          });
          if (res.code === 200) {
