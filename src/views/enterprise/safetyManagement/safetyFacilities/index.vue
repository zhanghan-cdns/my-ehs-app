...
import {
  safetyFacilitiesListAPI,
  safetyFacilitiesDeleteAPI,
...
export default {
  ...
  methods: {
    ...
    async getTableData({ query, domain }) {
      ...
      const res = await safetyFacilitiesListAPI(params);
      if (res.code === 200) {
    ...
    async deleteData(row) {
      this.$confirm("确定删除该条数据吗?", "提示", {
        ...
        .then(async () => {
          const res = await safetyFacilitiesDeleteAPI({
            ids: row.id,
          });
          if (res.code === 200) {
