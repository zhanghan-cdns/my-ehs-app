...
<script>
import {processCreateAPI, processViewAPI, processUpdateAPI} from '@/api/enterprise/process'
...
export default {
  ...
  methods: {
    ...
    async add() {
      try {
        const res = await processCreateAPI(this.form);
        if (res.code === 200) {
          this.close()
          this.$emit('refresh')
        }
    ...
    async modify() {
      try {
        const res = await processUpdateAPI({...this.form, id: this.rowData.id});
        if (res.code === 200) {
          this.close()
          this.$emit('refresh')
        }
