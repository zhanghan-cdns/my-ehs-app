...
import { saveProgressAPI } from "@/api/education/dailyTraining/onlineLearningAndExamination/onlineLearning";
...
export default {
  ...
  methods: {
    ...
    start() {
      let now = new Date();
      this.tipFlag = false;
      this.startTime = this.$dayjs(now).format("YYYY-MM-DD HH-mm:ss");
    ...
    open(data) {
      ...
      this.id = data.id;
      this.timeHandle(data.learnDuration);
      let params = {
    ...
    async saveProgress() {
      clearInterval(this.timer);
      let now = new Date();
      this.endTime = this.$dayjs(now).format("YYYY-MM-DD HH-mm:ss");
      let min =
        Number(this.hour * 60 + this.minute + Math.floor(this.second / 60)) -
        this.initMin;
      let params = {
