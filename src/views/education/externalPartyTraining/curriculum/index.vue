<template>
  <div>
    <list-container>
      ...
      <template #right>
        <div class="page_title">外来方培训学习课程</div>
        ...
        <div class="curriculum_list" v-if="listData.length > 0">
          <div class="curriculum_item" v-for="item in listData" :key="item">
            ...
            <div class="flex_box">
              ...
              <div class="half curriculum_type">
                <span>课程类型：</span>
                <span style="text-align: right; color: #4a80fc">三级教育</span>
              </div>
            </div>
            <div class="flex_box training_level">
              <div>
                <span>培训级别：</span>
                <span style="text-align: right; color: #4a80fc">{{
                  item.trainLevel
                }}</span>
              </div>
            </div>
    ...
    <edit-dialog ref="editDialogRef" :dict="{common_judge}"></edit-dialog>
  </div>
</template>
<script>
import { courseListAPI } from "@/api/education/threeLevel/curriculum";
import studyDialog from "../../threeLevel/curriculum/components/studyDialog.vue";
import testPaper from "../../threeLevel/curriculum/components/testPaper";
import editDialog from "./components/editDialog.vue";
import dict from "@/mixins/dict";
export default {
  components: { studyDialog,testPaper,editDialog},
  mixins:[dict],
  data() {
    return {
      noCurriculum: false,
  ...
  methods: {
    async getList() {
      ...
      this.listData.map((item) => {
        ...
        //判断该课程是否可以参加考试
        //是否绑定学时，是则需要全部学完才能考试；
        if (item.isExamBindHours == "UDGE:TRUE") {
          if (item.learnDuration == item.courseHours) {
            item.canExam = true;
          } else {
            item.canExam = false;
    ...
    //开始考试
    goExam(courseId) {
      this.$refs.testPaperRef.open(courseId);
    },
  },
};
</script>
